...
def add_day():
    ...
    # Solution file
    module_path = os.path.abspath(os.path.join(SOLUTIONS_DIR, f"year_{year}"))
    solution_file = os.path.join(module_path, f"day_{day:02}.py")
    create_module_dir(module_path)
    write_solution_template(solution_file, year, day)

    # Test file
    test_file = os.path.join(module_path, f"day_{day:02}_test.py")
    create_module_dir(module_path)
...
def read_solution_template(year: int, day: int) -> str:
    ...
    template_path = os.path.join(ROOT_DIR, "adventofcode/scripts/templates/day_template.txt")
...
def read_test_template(year: int, day: int) -> str:
    ...
    template_path = os.path.join(ROOT_DIR, "adventofcode/scripts/templates/test_template.txt")
